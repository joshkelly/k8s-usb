...
cp $PWD/bin/{kubelet,kube-proxy,docker*} /usr/bin

cp $PWD/units/{kubelet.service,kube-proxy.service,docker.service} /etc/systemd/system/
...
cp $PWD/conf/{kubelet.conf,kube-proxy.conf} /etc/kubernetes/node
